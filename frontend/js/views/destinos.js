...
window.abrirModalDestino = (nombre, ubicacion, categoria, descripcion, calificacion, imagen_url) => {
    ...
    const modalHtml = `
        <div id="modal-detalle-destino" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.6); display: flex; justify-content: center; align-items: center; z-index: 10000; padding: 20px;">
            <div style="background: white; border-radius: 12px; width: 100%; max-width: 600px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.3); position: relative;">
                ...
                <img src="${imagen_url}" alt="${nombre}" style="width: 100%; height: 280px; object-fit: cover;">
                
                <div style="padding: 25px;">
                    <h2 style="margin: 0 0 10px 0; color: #333; font-size: 24px;">${nombre}</h2>
                    <p style="margin: 0 0 15px 0; color: #ef4444; font-weight: bold; font-size: 14px; text-transform: uppercase;">${ubicacion} • Categoría: ${categoria}</p>
                    ...
                    <div style="background: #f1f8e9; padding: 12px 15px; border-radius: 6px; border-left: 5px solid #7cb342; margin-bottom: 20px;">
                        <span style="font-weight: bold; color: #33691e;">Calificación de usuarios:</span> 
                        <span style="color: #2e7d32; font-weight: bold;">${calificacion ? calificacion + '/10' : 'No calificado'}</span>
                    </div>
...
export async function destinos() {
    ...
    try {
        ...
        return `<div style="text-align:center; padding: 50px;"><h3>Error al cargar los destinos. Verifica tu conexión a internet o las políticas RLS de Supabase.</h3></div>`;
    }

    // 1. Agrupar por municipio (Usando 'ubicacion' que es la columna real)
    const barranquilla = listado.filter(d => d.ubicacion === "Barranquilla");
    const puerto = listado.filter(d => d.ubicacion === "Puerto Colombia");
    const tubara = listado.filter(d => d.ubicacion === "Tubará");
    const usiacuri = listado.filter(d => d.ubicacion === "Usiacurí");

    // 2. Definir palabras clave para subcategorías
    ...
    const generarTarjetas = (arreglo) => {
        return arreglo.map(d => `
            <div class="tarjeta-destino" style="background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1); display: flex; flex-direction: column;">
                <img src="${d.imagen_url}" style="width: 100%; height: 180px; object-fit: cover;">
                <div style="padding: 15px; flex-grow: 1; display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <button onclick="abrirModalDestino('${d.nombre.replace(/'/g, "\\'")}', '${d.ubicacion}', '${d.categoria}', '${d.descripcion.replace(/'/g, "\\'")}', '${d.calificacion}', '${d.imagen_url}')" style="margin-top: 15px; background: linear-gradient(135deg, #ef4444, #dc2626); color: white; border: none; padding: 10px 12px; border-radius: 6px; cursor: pointer; font-weight: bold; width: 100%; box-shadow: 0 4px 6px rgba(239, 68, 68, 0.2);">
                        Ver detalle
                    </button>
                </div>
            </div>
