        ...
        <div id="modal-detalle-destino" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.6); display: flex; justify-content: center; align-items: center; z-index: 10000; padding: 20px;">
            <div style="background: white; border-radius: 12px; width: 100%; max-width: 600px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.3); position: relative;">
                ...
                <div style="padding: 25px;">
                    ...
                    <p style="margin: 0 0 15px 0; color: #0288d1; font-weight: bold; font-size: 14px; text-transform: uppercase;">${municipio} • Categoría: ${categoria}</p>
                    ...
                    <button onclick="document.getElementById('modal-detalle-destino').remove()" style="width: 100%; background: #0288d1; color: white; border: none; padding: 12px; border-radius: 6px; font-size: 16px; cursor: pointer; font-weight: bold;">
                        Cerrar Ventana
                    </button>
                </div>
            </div>
...
export async function destinos() {
        ...
        return arreglo.map(d => `
            <div class="tarjeta-destino" style="background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1); display: flex; flex-direction: column;">
                ...
                <div style="padding: 15px; flex-grow: 1; display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    <button onclick="abrirModalDestino('${d.nombre}', '${d.municipio}', '${d.categoria}', '${d.descripcion.replace(/'/g, "\\'")}', '${d.eficiencia}', '${d.foto}')" style="margin-top: 15px; background: #0288d1; color: white; border: none; padding: 8px 12px; border-radius: 4px; cursor: pointer; font-weight: bold; width: 100%;">
                        Ver detalle
                    </button>
                </div>
            </div>
    ...
    <div style="padding: 30px; max-width: 1200px; margin: 0 auto; font-family: sans-serif;">
        <h2 style="text-align: center; color: #222; margin-bottom: 30px; font-size: 28px;">Destinos Turísticos del Atlántico</h2>
        
        <!-- SECCIÓN BARRANQUILLA -->
        <h3 style="color: #0288d1; border-bottom: 2px solid #0288d1; padding-bottom: 5px; margin-top: 20px; margin-bottom: 15px;">Barranquilla</h3>
        ...
        <h3 style="color: #0288d1; border-bottom: 2px solid #0288d1; padding-bottom: 5px; margin-bottom: 15px;">Puerto Colombia</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 20px; margin-bottom: 40px;">
            ${generarTarjetas(puerto)}
        </div>

        <!-- SECCIÓN TUBARÁ -->
        <h3 style="color: #0288d1; border-bottom: 2px solid #0288d1; padding-bottom: 5px; margin-bottom: 15px;">Tubará</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 20px; margin-bottom: 40px;">
            ${generarTarjetas(tubara)}
        </div>

        <!-- SECCIÓN USIACURÍ -->
        <h3 style="color: #0288d1; border-bottom: 2px solid #0288d1; padding-bottom: 5px; margin-bottom: 15px;">Usiacurí</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 20px; margin-bottom: 40px;">
